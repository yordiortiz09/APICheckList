...
def generar_pdf(datos: dict, articulos: list, campos: dict, clave_pedido: str, referencia_pedido: str, fecha: str, hora: str, sucursal: str, recolector: str) -> bytes:
    ...
    pdf.add_labeled_line("NOMBRE DE LA MASCOTA:", safe_str(datos.get("NOMBRE DE LA MASCOTA")))
    pdf.set_font("Arial", "B", 9)
    pdf.cell(85, 7, "VETERINARIO QUE LA ATIENDE REGULARMENTE:", 1)
    pdf.set_font("Arial", "", 9)
    pdf.cell(105, 7, safe_str(datos.get("VETERINARIO")), 1, ln=True)

    pdf.set_font("Arial", "B", 9)
    pdf.cell(25, 7, "RAZA:", 1)
    pdf.set_font("Arial", "", 9)
    pdf.cell(60, 7, safe_str(datos.get("RAZA")), 1)
    pdf.set_font("Arial", "B", 9)
    pdf.cell(25, 7, "PESO:", 1)
    pdf.set_font("Arial", "", 9)
    pdf.cell(80, 7, safe_str(datos.get("PESO")), 1, ln=True)

    pdf.set_font("Arial", "B", 9)
    pdf.cell(25, 7, "EDAD:", 1)
    pdf.set_font("Arial", "", 9)
    pdf.cell(35, 7, safe_str(datos.get("EDAD")), 1)
    pdf.set_font("Arial", "B", 9)
    pdf.cell(40, 7, "CAUSA DE MUERTE:", 1)
    pdf.set_font("Arial", "", 9)
    pdf.cell(90, 7, safe_str(datos.get("CAUSA DE MUERTE")), 1, ln=True)

    pdf.add_labeled_line("DUEÑO O CONTRATANTE:", safe_str(datos.get("DUEÑO O CONTRATANTE")))
    pdf.add_labeled_line("DOMICILIO:", safe_str(datos.get("DOMICILIO")))
    pdf.add_labeled_line("TELEFONO(S):", safe_str(datos.get("TELEFONO(S)")))
    pdf.add_labeled_line("¿CÓMO SUPO DE NOSOTROS?", safe_str(datos.get("¿CÓMO SUPO DE NOSOTROS?")))
    pdf.add_labeled_line("LUGAR DE RECOLECCION:", safe_str(datos.get("LUGAR DE RECOLECCION")))
    ...
    pdf.set_font("Arial", "", 9)
    pdf.multi_cell(125, 20, safe_str(datos.get("ESPECIFICACIONES")), border=1)
    pdf.set_y(y_inicio + 20)

    pdf.add_labeled_line("DUEÑO, DUEÑA O FAMILIA:", safe_str(datos.get("FAMILIA")))
